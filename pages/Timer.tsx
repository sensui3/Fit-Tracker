...
const Timer: React.FC = () => {
  ...
  const formatTime = (ms: number) => {
    ...
    const milliseconds = Math.floor((ms % 1000) / 100); // Changed to 100 for 1 digit milliseconds as per visual adjustment if needed, but keeping 10/100 logic consistent

    return {
      min: Math.max(0, minutes).toString().padStart(2, '0'),
      sec: Math.max(0, seconds).toString().padStart(2, '0'),
      ms: Math.max(0, milliseconds).toString().padStart(2, '0') // Keeping 2 chars
    };
  };
  ...
  return (
    <div className="flex-grow flex flex-col items-center justify-start py-8 px-4 sm:px-6 lg:px-8">
      {/* Page Heading */}
      <GranularErrorBoundary name="TimerHeader">
        <div className="w-full max-w-4xl text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Foco Total</h1>
          <p className="text-slate-500 dark:text-gray-400 text-lg">Cronômetro de alta precisão para seus treinos.</p>
        </div>
      </GranularErrorBoundary>

      {/* Timer Card */}
      <GranularErrorBoundary name="TimerMainCard">
        <div className="w-full max-w-3xl bg-white dark:bg-surface-dark rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none border border-slate-100 dark:border-[#2a402a] p-6 sm:p-10 flex flex-col items-center gap-8">
          ...
          {/* Segmented Control */}
          <div className="w-full max-sm background-slate-100 dark:bg-[#0a160a] p-1.5 rounded-xl flex items-center mb-2">
            <button
              onClick={() => {
                if (isRunning) return;
                setMode('stopwatch');
      ...
      {/* Laps History */}
      <GranularErrorBoundary name="TimerLapsHistory">
