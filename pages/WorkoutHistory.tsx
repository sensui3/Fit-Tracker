...
const WorkoutHistory: React.FC = () => {
    ...
    const loadWorkouts = useCallback(async () => {
        ...
        try {
            ...
            const countResult = await dbService.query`
                ...
                AND (notes ILIKE ${`%${searchTerm}%`} OR EXISTS (
                    SELECT 1 FROM training_plans tp 
                    WHERE tp.id = workout_sessions.plan_id 
                    AND tp.name ILIKE ${`%${searchTerm}%`}
                ))
            `;
            const total = parseInt(countResult[0]?.total || '0');
            setTotalItems(total);

            // 2. Get paginated workouts
            const workoutsData = await dbService.query`
                ...
                AND (ws.notes ILIKE ${`%${searchTerm}%`} OR tp.name ILIKE ${`%${searchTerm}%`})
            ...
            const mappedWorkouts = workoutsData.map((workout: WorkoutSessionRecord) => {
                ...
                return {
                    ...
                    title: workout.plan_name || 'Treino Livre',
                    type: 'Musculação',
    ...
    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto flex flex-col gap-8">
            {/* Header */}
            <GranularErrorBoundary name="WorkoutHistoryHeader">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 flex-1 max-w-lg">
                        <Input
                            placeholder="Buscar por treino ou notas..."
                            value={searchTerm}
            ...
            {/* History List */}
            <GranularErrorBoundary name="WorkoutHistoryList">
                    ...
                    {loading ? (
                        ...
                        <div className="py-20 text-center">
                            ...
                            <p className="text-slate-500">
                                {searchTerm ? 'Nenhum resultado para sua busca.' : 'Seu histórico de treinos aparecerá aqui assim que você concluir sua primeira sessão.'}
                            </p>
                            {searchTerm && (
                                <button
